...
export function loadAgentCallbackConfig(
...
function parseCallbackToken(value: string | undefined): string | undefined {
  ...
  if (/\r|\n/.test(value)) {
    throw new Error("DEVSPACE_AGENT_CALLBACK_TOKEN must not contain newlines.");
  }
  return value;
}
...
function callbackErrorMessage(error: unknown, timeoutMs: number): string {
  ...
  if (error instanceof Error && error.name === "AbortError") {
    return `Request aborted after ${timeoutMs}ms`;
  }
...
function boundedError(message: string): string {
  return message.length <= MAX_PERSISTED_ERROR_LENGTH
    ? message
    : `${message.slice(0, MAX_PERSISTED_ERROR_LENGTH - 3)}...`;
}
